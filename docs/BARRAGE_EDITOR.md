...
편집기에는 각기 다른 회피 감각을 시험할 수 있는 기본 패턴 11종이 들어 있다.

- `팡팡 - 가시 삼연발`: 플레이어 조준의 기본형
- `쌍류 나선`: 서로 반대로 회전하는 두 나선의 흐름 읽기
- `떠도는 해류문`: 가로·세로 탄벽의 움직이는 빈틈 찾기
- `심해 별자리 맥동`: 속도가 다른 이중 링 사이 타이밍 잡기
- `기뢰꽃 정원`: 지연 폭발 위치를 예측하는 영역 제어
- `엇갈린 폭우`: 위아래 사선 흐름을 따라 이동하기
- `가시 추 왕복`: 왕복하는 부채꼴을 크게 피한 뒤 미세 조정하기
- `미끼와 작살`: 느린 유도탄 뒤의 빠른 추격탄에 대응하기
- `해마 씨앗 곡선`: 굽어 날아가는 모탄이 이동 중 자탄을 살포하기
- `등대 레이저 소인`: 예고선 뒤 회전하는 직선 레이저 피하기
- `멈춤 뒤 재조준`: 탄이 정지한 뒤 플레이어를 다시 겨누고 돌진하기
...
| 직선 레이저 `laser` | 예고 후 선분 전체를 공격 | 길이, 폭, 예고·공격·소멸 시간, 회전속도 |
...
발사기마다 아래 값을 조합해 직선 탄을 새 궤적으로 바꿀 수 있다.

- `acceleration`, `maxSpeed`: 가속·감속과 최고 속도
- `angularVelocity`: 계속 한쪽으로 휘는 곡선
- `waveAmplitude`, `waveFrequency`: 진행 방향을 좌우로 흔드는 물결
- `homingTurnRate`, `homingDuration`: 일정 시간만 플레이어를 따라가는 제한 유도

### 시간 행동

탄이 태어난 뒤의 초 단위 시각 `at`에 행동을 실행한다. `repeat`와 `interval`로 같은 행동을 반복할 수 있으며, 한 발사기에는 최대 8개를 둔다.

| 행동 | 결과 |
|---|---|
| `changeSpeed` | 즉시 또는 `duration` 동안 속도 변경 |
| `changeDirection` | 절대·상대 각도 또는 플레이어 조준 방향으로 전환 |
| `spawn` | 현재 위치에서 부채·링 형태의 자탄 생성 |
| `vanish` | 모탄 소멸 |

자탄은 최대 2단계 깊이와 프레임당 생성 예산으로 제한해 잘못된 JSON이 탄을 무한 증식시키지 못하게 한다. 레이저는 `laserTelegraph` 동안 예고만 하고, `laserActive` 구간에만 충돌하며, `laserFade` 동안 무해하게 사라진다.
