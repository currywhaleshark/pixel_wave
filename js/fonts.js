...
const Fonts = {
  ...
  faces: [
    { family: "'Galmuri9'",  native: 9,  ok: false },
    { family: "'Galmuri11'", native: 11, ok: false },
  ],
  fallback: 'sans-serif',
  loaded: false,
  ...
  f(px, bold) {
    ...
    for (const face of this.faces) {
      ...
      // 동점이면 고유 크기가 큰 쪽(=배수가 작은 쪽)이 더 또렷하다
      if (diff < bestDiff || (diff === bestDiff && face.native > best.native)) {
        best = face; bestDiff = diff; best.size = size;
      }
    }
    return best ? `${best.size}px ${best.family}` : `${Math.round(px)}px ${this.fallback}`;
  },
};
